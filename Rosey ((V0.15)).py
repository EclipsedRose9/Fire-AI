        ...
        for code_needed, used_list in usage.items():
            if used_list is None:
                ...
                for (stn_obj, qty_taken, dist) in used_list:
                    CurrentData.append(f"{qty_taken} {code_needed} from {stn_obj['station']} ETA {cost_here} minuets")
    else:
        print("No usage results (maybe no codes were needed).")

    CurrentData = ("Rosey says: Theres a fire at "+Adress+" "+Borough+" "+Postcode+" I recommend "+str(CurrentData))
    RoseyText.config(text=CurrentData)
