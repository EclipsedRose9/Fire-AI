...
def Logic():
    ...
    CurrentDataString = "\n".join(
    f"{CurrentData[i]} {CurrentData[i+1]}" if i+1 < len(CurrentData) else CurrentData[i]
    for i in range(0, len(CurrentData), 4)
)
    CurrentDataString = ("Rosey says: Theres a fire at "+Adress+" "+Borough+" "+Postcode+" I recommend: "+str(CurrentDataString))
...
def find_all_units(user_half_postcode, stations, graph, requirements):
    """
    requirements = [('P',2), ('PL',1), ('FRU',1), ...]
    returns usage dict + naive time
    """
    # Precompute BFS dist
    stn_info = []
    for stn in stations:
    ...
    for (code_needed, qty_needed) in requirements:
        ...
        for info in stn_info:
            ...
            cost_here = (info['dist'] + 1)*5 if info['dist'] is not None else 9999
            if needed <= 0:
                break
        if needed > 0:
