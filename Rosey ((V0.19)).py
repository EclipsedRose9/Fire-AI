...
def Logic():
    ...
    CurrentDataString = ("Rosey says: There's a fire at " + Adress + " " + Borough + " " + Postcode +
                         " I recommend:\n" + CurrentDataString)
...
def schedule_station_updates(delayed_updates, final_address):
    ...
    for (station_name, unit_code, _, eta_time) in delayed_updates:
        ...
        UpdateText = (station_name +" " + unit_code + " has arrived at " + final_address)
        root.after(eta_time, lambda: print(UpdateText))
        root.after(eta_time, lambda: RoseyText.configure(state='normal'))
        root.after(eta_time, lambda: RoseyText.delete('1.0', END))
        root.after(eta_time, lambda: RoseyText.insert("end", "".join(UpdateText)))
        root.after(eta_time, lambda: RoseyText.configure(state='disabled'))
